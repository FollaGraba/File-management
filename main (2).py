...
class App(tk.Tk):
    def __init__(self):
        ...
        self.percent_label = ttk.Label(self, text='choisir les fichiers fasta et le fichier des mots puis cliquer sur commencer')
        self.percent_label.configure(background='white')
    ...
    def run(self):
        ...
        def find_occurrence_per_sequence(sequences, list_of_words):
            ...
            for word in list_of_words:
                occurrence[word] = {}
                progress = int(list_of_words.index(word) / len(list_of_words) * 100)
                self.progress['value'] = progress
                self.percent_label['text'] = str(progress) + " %"
                self.label_info['text'] = 'Processing word : ' + word
                self.update()
        ...
        def find_most_frequent(occurrence):
            ...
            for key in occurrence:
                ...
                i = i+1
                self.progress['value'] = progress
                self.label_info['text'] = str(progress) + " words parrcourus"
                self.percent_label['text'] = "comparaison d'occurence dans la sequence : " + key
                self.update()
